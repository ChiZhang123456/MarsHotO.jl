...
LAB 是 laboratory frame 的缩写，中文通常称为实验室参考系。

在火星问题中，LAB 不是地球上的实验室。它表示相对于火星大气或火星中心建立的参考系。MGITM 的高度、热 O 的上升和下降、火星重力、逃逸速度以及最后画出的能量，都在这个参考系中描述。

可以把 LAB 想象成一个站在火星大气旁边、不随两个碰撞粒子一起移动的观察者。
...
当前 Ali 系综模拟采用静止靶近似，即 $\mathbf v_2=0$。通用两体碰撞公式允许 $\mathbf v_2$ 非零，以后也可以从中性温度对应的 Maxwell 分布抽取靶速度。

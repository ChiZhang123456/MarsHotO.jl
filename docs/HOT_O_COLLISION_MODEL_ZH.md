...
MarsHotO 的通用两体函数允许 $\mathbf v_2\ne0$。旧的单粒子接口可以根据 MGITM 中性温度 $T_n$ 抽取目标粒子的 Maxwell 速度：
...
严格复现 Ali 十步算法的 `run_hot_o_corona` 使用静止目标，即 $\mathbf v_2=0$。这样，O 与 O 碰撞后的次级 O 能量只来自入射热 O 的能量传递。
...
完整 Ali ensemble 使用静止目标，因此热 O 的 LAB 动能同时也是相对碰撞能量。
...
等质量碰撞的能量传递更有效。通用两体函数如果传入非零目标热速度，最终结果会发生相应变化。
...
6. 通用两体函数允许目标具有任意速度，但是完整 Ali ensemble 使用静止目标。
7. 主输运目前只继续追踪入射热 O。

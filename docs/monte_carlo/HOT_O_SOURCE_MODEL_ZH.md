...
4. 将得到的 $Q_k(z)$ 输入热原子传输模型。

当前 MarsHotO 在此基础上显式加入了给定的 $\mathrm{O_2^+}$ 振动态布居。源模型产生的是初始粒子，后续碰撞不会反过来改变源项，而是把 $Q_k(z)$ 转换为 O 冕的高度、能量和速度分布。
...
$Q_k(z)$ 由化学源模型得到，O 冕分布必须通过热 O 传输 Monte Carlo 模型计算。

## 11. 输运 Monte Carlo 的宏粒子权重

默认源高度为 100 至 250 km，间隔为 1 km。每个高度生成
...
```math
w_i=
\frac{Q_{\mathrm{hotO}}(z_i)V_i}{N_i}.
```

$w_i$ 的单位是 $\mathrm{s^{-1}}$，表示该模拟粒子代表每秒产生的真实
热 O 数量。同一源高度的粒子权重相同，不同源高度的权重通常不同。

每个源粒子的能量由该高度的初生能量分布随机抽取，方向为各向同性。粒子
随后在火星重力下运动，并经历与背景中性大气的随机碰撞。如果碰撞靶粒子
是 O，反冲 O 作为次级热 O 加入队列，并继承母粒子的 $w_i$。

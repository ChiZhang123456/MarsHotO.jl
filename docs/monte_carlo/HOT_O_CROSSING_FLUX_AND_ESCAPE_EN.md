...
The GIF below uses the same snapshot flux definition and shows the evolution from $t=0$ to $100\ \mathrm{s}$. The snapshot cadence is 2 s, giving 51 frames. Every frame uses the same colorbar limits, and the playback rate is 10 frames per second.

![Animated hot O altitude and energy snapshot flux](../../examples/figures/hot_o_energy_altitude_time_evolution.gif)

The animation describes an initially released ensemble in which all primary particles are injected at $t=0$. It is not a steady state distribution generated by continuous source injection. First generate the local frame data:
...
The fourth and fifth arguments are the snapshot cadence and maximum snapshot time in seconds. Then create the GIF:
...
The approximately 48 MB frame table remains local and is not committed to GitHub.

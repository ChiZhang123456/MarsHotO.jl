...
| DR events per altitude in each batch | 500 |
| Number of independent batches | 20 |
| Total DR events | 1,510,000 |
| Total primary O particles | 3,020,000 |
| Total secondary O particles | 2,981,617 |
| Total tracked particles | 6,001,617 |
| Total crossing and terminal events | 197,188,146 |
...
The physical dissociative-recombination event rate in that shell is
...
If $N_{\mathrm{event},i}^{\mathrm{MC}}$ Monte Carlo reaction events are generated at that altitude, each event has rate weight
...
Each simulated reaction produces two O atoms. Both atoms inherit $w_{\mathrm{event},i}$, so their summed source weight equals the physical hot O production rate $2\dot N_{\mathrm{event},i}$. A recoil secondary O inherits the same rate weight as its parent.
...
This zero bulk velocity does not set the COM velocity of an individual reaction to zero. Each event samples independent electron and O2+ thermal velocities, and its COM velocity is calculated from those sampled velocities.
...
The GIF below shows the instantaneous directional number-density spectrum $n(E,z,t)$ from 100 to 400 km. The left panel contains upward particles with positive Mars-centered radial velocity, and the right panel contains downward particles. It uses 10,000 dissociative-recombination events per 1 km source altitude. Snapshots extend from $t=0$ to $100\ \mathrm{s}$ at 5 s cadence, giving 21 frames. Both panels and all frames use the same colorbar range. Energy is displayed from 0 to 6 eV.

![Animated upward and downward hot O altitude-energy density](../../examples/figures/hot_o_energy_altitude_time_evolution.gif)

The animation describes an initially released ensemble in which all primary particles are injected at $t=0$. It is not a steady-state distribution generated by continuous source injection, and it is not the altitude-crossing flux shown below. Generate the directional frame data with:
...
The frame table remains local under `examples/output/` and is not committed to GitHub.
...
The black curve is the total bidirectional crossing flux
...
This total is distinct from the net upward flux
$\Phi_{\mathrm{up}}-\Phi_{\mathrm{down}}$.
...
1. Number of batches
2. Number of DR events per source altitude in each batch
3. First random seed
4. Local output directory

There are 151 source altitudes. The total numbers of DR events and primary O atoms are

```math
20\times500\times151
=
1{,}510{,}000\ \mathrm{events},
\qquad
...
The raw binary event files are approximately 16.2 GB and remain local. GitHub contains the complete calculation code, fixed inputs, reproduction commands, and final figures.
...
| `examples/animate_hot_o_time_snapshots.py` | Render fixed cadence snapshot flux as a GIF animation |
| `examples/run_directional_hot_o_time_snapshots.jl` | Generate parallel upward and downward density snapshots from paired DR events |
| `examples/animate_directional_hot_o_time_snapshots.py` | Render the directional density snapshots as the published GIF |
| `test/runtests.jl` | Tests of Maxwellians, cross sections, scattering, conservation, and reproducibility |

## Complete output set

A steady-state corona run writes total and directional residence-time density
tables, the altitude-dependent DR event weights, and a TOML run summary through
`write_corona_outputs`. The crossing-event postprocessor writes upward,
downward, total, and net upward fluxes, their batch standard errors, a compressed NPZ
copy, the 300 km escape summary, normalization metadata, and the corresponding
figures. Large particle-level binary event streams remain local under
`examples/output/`.
